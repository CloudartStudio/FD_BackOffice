                ...
                <div className={style.TableBase}>
                    ...
                    <div className={style.TableBodyBox}>
                        {body_data.map((tr, index) => {
                            const cellTd = [tr.ID, tr.ragione_sociale, tr.partita_iva, tr.codice_sdi, tr.telefono, tr.email, tr.is_b2b, tr.is_b2c];
                            return (
                                <div className={index % 2 == 0 ? style.TableBaseRowWhite : style.TableBaseRowBlue}>
                                    {cellTd.map((td) => {
                                        if (td === true) return <div className={style.TableBaseCell}>🟩</div>;
                                        if (td === false) return <div className={style.TableBaseCell}>🟥</div>;
