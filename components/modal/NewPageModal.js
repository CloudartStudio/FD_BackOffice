  ...
  useEffect(() => {
    const fetch = async () => {
      if ( id ) {
        ...
        const { Nome, Link, RelatedSections, IsActive, IsAgenzia, MinRole } =
          resposePage.data;
  ...
  const handleSavePage = async () => {
    if (isInEdit) {
      ...
      const response = await axios.put(
        "http://localhost:3000/api/manage/dpage",
        {
          Page: {
                          ...
                          return (
                            <div className={PageEditorStyle.PreviewSection}>
                              {section.data.map((element) => {
                                return (
                                  <div
                                    className={PageEditorStyle.ConfigSection}
                                  >
                                    <div
                                      onDrop={(e) => {
                                        handleDropDirection(
