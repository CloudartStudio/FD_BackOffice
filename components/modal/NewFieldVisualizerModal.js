...
export default function NewFieldVisualizerModal({ isOpen, onActionCloseModal, SectionID }) {
    ...
    const HandleSendNewFieldPostRequest = async () => {
        ...
        const response = await axios.post("http://localhost:3000/api/visualizer/fieldbase", {
            newConfig: newConfig,
        });
        NotificationCtx.showNotification({
            title: "Salvataggio",
                    ...
                    <div className={style.Modal}>
                        ...
                        <div className={style.ModalBody}>
                            <div className={style.ModalField}>
                                <label>Label 1</label>
                                <br />
                                <input ref={label1_Input} type={"text"} placeholder="label1..." name="label1"></input>
                            </div>
                            <div className={style.ModalField}>
                                <label>Info</label>
                                <br />
                                <input ref={info_Input} type={"text"} placeholder="info..." name="info"></input>
                            </div>
                            <div className={style.ModalField}>
                                <label>Icon name</label>
                                <br />
                                <input ref={iconid_Input} type={"text"} placeholder="iconid..." name="iconid"></input>
                            </div>
                            <div className={style.ModalField}>
                                <label>Value info (es: €/$)</label>
                                <br />
                                <input ref={valueinfo_Input} type={"text"} placeholder="value info..." name="valueinfo"></input>
                            </div>
                            <div className={style.ModalField}>
                                <label>Return name</label>
                                <br />
                                <input ref={returnName_Input} type={"text"} placeholder="returnName info..." name="returnName"></input>
                            </div>
                            <div className={style.FullModalFieldSection}>
                                <h3>Query</h3>
                                <br />
                                <textarea ref={Query_Input} placeholder="Select * From ..." cols={150} rows={10}></textarea>
                            </div>
                        </div>
                        <div className={style.ModalFoot}>
                            <button onClick={HandleSendNewFieldPostRequest} className={style.Success}>
                                INVIA
                            </button>
                        </div>
                    </div>
